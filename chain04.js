/* Estenda o sistema de processamento de pagamentos apresentado anteriormente (código chain02.js)
para incluir uma nova etapa chamada EtapaDeteccaoFraude, responsável por verificar possíveis fraudes na
transação. */
...
class EtapaDeteccaoFraude extends EtapaProcesso {
    processar(pagamento) {
        console.log("[05] Verificação de fraude...");
        if (pagamento.valor > 1000){ // lógica para caso de sucesso de identificação de fraude
            console.log("[--FAIL--] Fraude identificada. Processo encerrado.");
        }else{ // tratativa para verificação validada
            console.log("[--OK--] Verificação validada.");
            if (this.proximaEtapa) {
                this.proximaEtapa.processar(pagamento);
            }
        }
...
/* uso (vai dar erro)
const cliente = new Cliente;
cliente.iniciarProcessoPagamento(1500); */